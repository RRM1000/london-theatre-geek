...
async function getShowDataBySlug(slug: string) {
    ...
    let imageUrl = showRaw.fieldData.imageurl || `https://via.placeholder.com/400x600/18181b/f43f5e.png?text=${encodeURIComponent(showRaw.fieldData.name || 'Show')}`;
    ...
    const similarShows = similarRaw.map((item: any) => {
        let simImageUrl = item.fieldData.imageurl || `https://via.placeholder.com/400x600/18181b/f43f5e.png?text=${encodeURIComponent(item.fieldData.name || 'Show')}`;
        return {
            ...item.fieldData,
        ...
        <div className="space-y-12 pb-24">
            ...
            {similarShows.length > 0 && (
                <section className="max-w-4xl mx-auto mt-12">
                    <h2 className="text-2xl font-bold mb-6 text-white border-b border-zinc-800 pb-3 flex items-center gap-2">
                        <svg width="24" height="24" className="w-6 h-6 text-rose-500" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                        {similarShows.map((sim: any) => (
                            <a key={sim.slug} href={`/shows/${sim.slug}`} className="group flex bg-zinc-800/40 border border-zinc-700/30 rounded-xl overflow-hidden hover:border-rose-500/30 hover:bg-zinc-800/60 transition-all hover:-translate-y-1 shadow-lg hover:shadow-rose-500/10 h-32">
                                <div className="w-1/3 shrink-0 relative">
                                    <img
                                        ...
                                        loading="lazy"
                                    />
                                    <div className="absolute top-2 right-2 bg-zinc-900/90 backdrop-blur-md px-1.5 py-0.5 rounded text-[10px] font-medium text-amber-400 border border-amber-400/20">
                                        {sim.category}
                                    </div>
                                </div>
                                <div className="p-4 flex flex-col justify-center flex-1">
                                    <h3 className="font-bold text-base text-zinc-100 group-hover:text-rose-400 transition-colors line-clamp-1">{sim.name}</h3>
                                    <p className="text-sm text-zinc-400 mt-0.5 line-clamp-1 flex items-center gap-1">
                                    ...
                                    <div className="mt-2 text-sm flex items-center">
                                        <span className={sim.customData?.['Cheapest Price'] ? "text-zinc-500 line-through text-xs" : "text-rose-400 font-medium"}>
                                            {sim.pricerange}
                                        </span>
                                        {sim.customData?.['Cheapest Price'] && (
                                            <span className="ml-2 text-emerald-400 font-bold text-sm">
                                                {sim.customData['Cheapest Price']}
                                            </span>
                                        )}
                                    </div>
